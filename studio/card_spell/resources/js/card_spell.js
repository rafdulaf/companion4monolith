var CardSpell = {
    _i18n: {
        ...
        'en': {
            ...
            'saturation': "Saturation",
            'saturationPh': "?",
            'reaction': "Can be thrown in reaction",
    ...
    _getDisplayCardsCode: function(withEditLink)
    {
        ...
        if (!withEditLink)
        {
            ...
            for (var i in cards)
            {
                html += "<img id=\"spell-back-" + i + "\" style=\"display: none\" class=\"back\" src=\"studio/card_spell/resources/img/back.png\"/>"
            }
            html += "</div>";
        }
    ...
    _cardCode: function(card) {
        var code = "<div class=\"spell card\">"
                + "<img class=\"background\" src=\"studio/card_spell/resources/img/background.png\"/>";
        ...
        if (hasSaturation)
        {
            code += "<div class='cost-and-saturation'><img src='studio/card_spell/resources/img/slash2.png'/></div>";
            code += "<div class=\"saturation\">" + card.saturation + "</div>";
        }

        if (card.explosion)
        {
            code += "<div class='explosion" + (card.reaction ? " and-other" : "") + "'><img src='studio/card_spell/resources/img/explosion.png'/></div>";
        }
        if (card.reaction)
        {
    ...
    {
        text = text.replace(/\{(.*?)\}/g, "<img src='resources/img/$1.png'/>");
        return text;
    },
